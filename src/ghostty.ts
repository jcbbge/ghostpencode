...
export function readGhosttyTheme(themeName: string): Palette | null {
  ...
  let opencodeFilename: string | null = null;

  for (const line of lines) {
    const trimmed = line.trim();

    // Extract OpenCode filename from comment
    if (trimmed.startsWith('# OpenCode filename:')) {
      opencodeFilename = trimmed.replace('# OpenCode filename:', '').trim();
      continue;
    }
  ...
  // Store original OpenCode filename if present
  if (opencodeFilename) {
    (result as any)._opencodeFilename = opencodeFilename;
  }
...
`;
  ...
  // Store OpenCode filename for round-trip conversion
  if (opencodeFilename) {
    content += `# OpenCode filename: ${opencodeFilename}\n`;
  }
